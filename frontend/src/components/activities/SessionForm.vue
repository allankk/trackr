    ...
    <form @submit.prevent="submitForm" class="mt-10">
      <div class="card mx-auto">
        <div class="mx-auto w-full md:w-3/4 flex flex-col justify-between items-center relative">
          ...
          <DatePicker class="shadow-none w-full px-0 py-0" v-model="calendarValue" name="calendar" :showIcon="true"
            dateFormat="dd.mm.yy" />
          <span v-if="formErrors.calendarValue"
            class="text-red-500 text-sm text-bold absolute right-0 -bottom-6 font-bold">{{ formErrors.calendarValue
            }}</span>
        </div>
        ...
        <div v-if="addedActivities.length" class="activity-card-container p-2 mb-10">
          <div v-for="(activity, index) in addedActivities" :key="index" class="p-2">
            <SessionActivityCard :activity="activity" @remove-activity="handleRemoveActivity" />
          </div>
        </div>
        <Divider />
        ...
        <Dialog class="absolute h-full md:h-auto top-0 md:top-24 w-screen md:w-1/2 2xl:w-1/4"
          header="add activity to session" v-model:visible="activityModalVisible" :modal="true" :draggable="false">
          <SessionAddActivityCard :activityGroups="activityGroups" :activityTypes="activityTypes"
            @closeModal="closeActivityModal" @add-activity-group="handleAddActivityGroup"
