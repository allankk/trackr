<template>
    <li :class="{ 'layout-root-menuitem': root, 'active-menuitem': isActiveMenu }">
        ...
        <router-link v-if="item.to && !item.items && item.visible !== false"
            :class="[item.class, { 'active-route': checkActiveRoute(item) }]" @click="itemClick($event, item, index)"
        ...
        <Transition v-if="item.items && item.visible !== false" name="layout-submenu">
            <ul v-show="root ? true : isActiveMenu" class="layout-submenu">
                <SideMenuItem v-for="(child, i) in item.items" :key="child" :index="i" :item="child"
                    :parentItemKey="itemKey" :root="false"></SideMenuItem>
            </ul>
        </Transition>
    </li>
</template>
